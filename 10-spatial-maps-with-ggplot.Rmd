...
(as in beer).  So, if you have the stomach for it, the R route for spatial data certainly
...
```
and, thought `ggspatial` is available on CRAN, I recommend you get the development version from GitHub:
```{r, eval=FALSE}
devtools::install_github("paleolimbot/ggspatial")
```

When learning the functions in this spatial-data ecosystem, it is often difficult to keep straight
which functions come from which packages, so I will often use namespace addressing to make that
explicit here.
...
spatial data with a nice syntax.  Basically, that package makes a new geom called `geom_spatial()` that
deals with plotting spatial points, lines, or polygons.
...
While here were 1514 features in `streams`
```{r}
length(streams)
```
...
You can think of rasters as digitial "images"---basically regular grids of pixels.  
...
The package `rgdal` has some facilities for dealing with rasters, I think; however I am 
partial to the `raster` package written by Robert Hijmans (who was a collaborator with 
...
it through R was not as straightforward as I would have hoped.  This rasters has multiple
layers.  It is a color image so it has Red, Green, Blue layers (or "bands" as they are often
called.)  Accordingly we will want to use the `brick` function to read it in as a "rasterBrick":
```{r}
...
boundaries.  This is very useful if you want only a piece of the background.  Let's
...
That is pretty cool.  It might have been nice to have downloaded a higher resolution raster, which is available, but would have been
quite large at the full, zoomed out scale.
